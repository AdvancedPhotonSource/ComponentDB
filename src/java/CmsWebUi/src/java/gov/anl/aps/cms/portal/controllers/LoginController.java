...
public class LoginController implements Serializable 
{
   ...
   /**
    ...
    public String login() 
    {
        //if (username != null && password != null && !username.isEmpty() && !password.isEmpty()) {
            //if (LdapUtility.validateCredentials(username, password)) {
 ...
 /***
            try {
            SessionUtility.configureFromProperties();
            MetaSchedulerApi api = SessionUtility.getMetaSchedulerApi();
            api.login(username, password);
            SessionManager sessionManager = new SessionManager();
            SkaiSession session = api.getSession();
            sessionManager.addSession(session);
            SessionUtility.setSessionManager(sessionManager);
            if (session.isAdminRole()) {
                SiteLoginThread siteLoginThread = new SiteLoginThread(username, password, this);
                siteLoginThread.run();
                loggedIntoMetaSchedulerAsAdmin = true;
                return "admin/meta_scheduler/home?faces-redirect=true";
            } else {
                loggedIntoMetaSchedulerAsUser = true;
                return "user/meta_scheduler/home?faces-redirect=true";
            }

        } catch (SkaiException ex) {
            loggedInAsAdmin = false;
            //SessionUtility.addErrorMessage("Login Error", ex.getErrorMessage());
            return (username = password = null);
        }
**/
    }
    ...
    public String dropAdminRole() 
    {
        loggedInAsAdmin = false;
        loggedInAsUser = true;
        return "/views/user/home?faces-redirect=true";        
    }
