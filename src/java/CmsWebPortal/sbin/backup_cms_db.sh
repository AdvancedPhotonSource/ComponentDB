...
# Usage:
#
# $0 [CMS_DB_NAME [CMS_DB_BACKUP_DIR]]
#
...
CMS_DB_BACKUP_DIR=$2
if [ -z $CMS_DB_BACKUP_DIR ]; then
    CMS_DB_BACKUP_DIR=/tmp/cms_db_backup.$timestamp
fi
backupFile=cms_db_backup.$timestamp.sql
fullBackupFilePath=$CMS_DB_BACKUP_DIR/$backupFile
...
echo
echo
echo "Using DB backup directory: $CMS_DB_BACKUP_DIR"

mkdir -p $CMS_DB_BACKUP_DIR
...
lockCnt=0
processingFile=$CMS_DB_BACKUP_DIR/process.txt
while [ $lockCnt -lt $nTableLocks ]; do
    ...
    cat $fullBackupFilePath | sed -n ${firstLine},${lastLine}p > $CMS_DB_BACKUP_DIR/populate_$dbTable.sql
done
rm -f $processingFile
