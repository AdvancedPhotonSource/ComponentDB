...
package ALHCrawlerDBLayer;

# This package provides routines for writing ioc record references
# data from alh config files to a MySQL database. 
...
our @ISA = qw(Exporter);
our @EXPORT = qw( update_db_tables set_test_mode set_home_dir set_decryption_key);
our @EXPORT_OK = qw();
our $VERSION = 1.00;
our $ALH = 2;
...
sub update_db_tables {
    ...
    LogUtil::log('info',"Starting db updates.");
    ...
    if (!$dbh) {
        $dbh = db_connect();
        if (!$dbh) {
            LogUtil::log('error',"Cannot connect to database.");
            LogUtil::log('error',"Exiting...");
            exit;
        }
    }

    mark_alh_records_as_old() if $firstTime;
    $firstTime = 0;

    add_new_alh_records ($alh_hits_ref);

    $dbh->commit; 

    LogUtil::log('info',"Finished db updates.");
    return;
}


# Private
sub mark_alh_records_as_old {

    LogUtil::log('info',"Started marking existing alh records as old.");

    if ($test_mode) {
        LogUtil::log('debug',"Test mode. No database changes.");
        return;
    }
    if (!$dbh) {
        $dbh = db_connect();
    ...
    LogUtil::log('debug',"Connected to database.");

    eval {
        my $update_rec_client_sql;
        $update_rec_client_sql = 'update rec_client set current_load=0 '
            ."where rec_client_type_id=$ALH and current_load=1";
    ...
    if ($@) {
        $dbh->rollback;
        LogUtil::log('error',"Update error: $@");
        return;
    }
    
    LogUtil::log('info',"Finished marking existing alh records as old.");
    return;
};


# Private
sub add_new_alh_records {
    my $alh_hits_ref = $_[0];
    my $prevFilename = "";
    ...
    LogUtil::log('info',"Started adding uri, and  rec_client records.\n");

    my $uri_id = -1;
    my $vuri_id = -1;

    # Loop over array items, writing to URI, vuri, and rec_client
    foreach $row (@$alh_hits_ref) {

        # items were added using following line
        # push (@pv_list,{$pv_name,$arg_count});
        # push (@alh_hits,{$filename,\@pv_list});
        ...
        eval {
            if ($filename ne $prevFilename ) {
                ...
                my $insert_uri_sql = 'insert into uri ' .
                    ...
                    'values (?, ?, ?, ' . get_current_date_time() . ', \'alhcrawler\')';
        ...
        if ($@) {
            $dbh->rollback;
            LogUtil::log('error',"Error adding uri and rec_client records: $@");
            LogUtil::log('error',"Exiting...");
            exit;
        }
    }
    LogUtil::log('info',"Finished adding uri, and  rec_client records.\n");
    return;
}
