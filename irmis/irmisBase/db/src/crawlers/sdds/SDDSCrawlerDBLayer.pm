...
our @ISA = qw(Exporter);
our @EXPORT = qw( all_save set_test_mode set_home_dir set_decryption_key);
our @EXPORT_OK = qw();
our $VERSION = 1.00;
our $ALH = 2;
...
sub all_save {
    ...
    LogUtil::log('debug',"SDDSCrawlerDBLayer: all_save entered");

    if ($test_mode) {
        LogUtil::log('debug',"SDDSCrawlerDBLayer: all_save finished (test mode)");
        return;
    }
    ...
    if (!$dbh) {
        $dbh = db_connect();
        if (!$dbh) {
            LogUtil::log('error',"SDDSCrawlerDBLayer: Cannot connect to DB. Exiting...");
            exit;
        }
    }
    ...
    if (!$app_id) {
        LogUtil::log('error',"SDDSCrawlerDBLayer: Unable to resolve app name $app_name. ");
        return;
    }

    mark_sdds_records_as_old($app_name, $app_id);

    add_new_sdds_records($app_name, $app_id, $sdds_pvs);

    $dbh->commit; 

    LogUtil::log('debug',"SDDSCrawlerDBLayer: all_save finished");
    return;
}
...
# Private
sub mark_sdds_records_as_old {

    my $app_name = $_[0];
    my $app_id = $_[1];

    LogUtil::log('debug',"SDDSCrawlerDBLayer: mark_sdds_records_as_old entered");
    ...
    LogUtil::log('debug',"SDDSCrawlerDBLayer: marking all $app_name sdds records as old");
    ...
    if ($@) {
        $dbh->rollback;
        LogUtil::log('error',"mark_sdds_records_as_old():$@");
        return;
    }
    
    LogUtil::log('debug',"SDDSCrawlerDBLayer: mark_sdds_records_as_old finished");
    return;
};


# Private
sub add_new_sdds_records {
    my $app_name = $_[0];
    my $app_id = $_[1];
    my $sdds_pvs = $_[2];
    ...
    foreach $filename (keys %$sdds_pvs) {

        LogUtil::log('verbose',"Adding $filename to uri table");
        $pvs = $sdds_pvs->{$filename};
        eval {
