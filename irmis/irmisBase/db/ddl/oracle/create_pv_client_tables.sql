...
create table rec_client
  (
    rec_client_id int not null primary key,
...
drop sequence seq_rec_client;
create sequence seq_rec_client;
...
create table vuri
   (
    vuri_id int not null primary key,
    uri_id int
   );

drop sequence seq_vuri;
create sequence seq_vuri;
...
create table vuri_rel
  (
    vuri_rel_id int not null primary key,
    parent_vuri_id int,
    child_vuri_id int,
    rel_info varchar(1000)
  );

drop sequence seq_vuri_rel;
create sequence seq_vuri_rel;
