 ...
 */
public class FilteredControlComponentTreeModel extends ComponentTreeModel {

    List iocComponentList = null;    // only show these iocs

    public FilteredControlComponentTreeModel() {
        super(ComponentRelationshipType.CONTROL);
    }

    public TreePath getTreePath(Component node) {
        List tempPath = ComponentService.getComponentPathToRoot(node, hierarchy, iocComponentList);
        if (tempPath == null)
            return null;
