 ...
 * potentially large hierarchy by iterating over a filtered set of candidates.
 */
public class ComponentPortTreeFindController {
    ...
    public ComponentPortTreeFindController(JFrame appFrame,
                                           JTree tree,
                                           int h,
                                           int cIndex, // 1-componentA 2-componentB
                                           CableModel m,
        ...
        findConfig.setComponentIndex(cIndex);
		...
		model.addCableModelListener( new CableModelListener() {
                public void modified(CableModelEvent e) {
                    if (e.getType() == CableModelEvent.REDRAW) {
                        findDialog.reset();
            ...
            QueuedExecutorUtil.execute(new SwingThreadWork(appFrame) {
                    public void doNonUILogic() {
                        ...
                        if (hierarchy == ComponentRelationshipType.HOUSING) {
                            FilteredHousingComponentPortTreeModel tModel = 
                                (FilteredHousingComponentPortTreeModel)tree.getModel();                
                            tModel.setRoomConstraint(null);
                        }
                        if (hierarchy == ComponentRelationshipType.CONTROL) {
                            FilteredControlComponentPortTreeModel tModel = 
                                (FilteredControlComponentPortTreeModel)tree.getModel();                
                            tModel.setIOCConstraint(null);
                        }
                        ComponentPortTreeModel gModel = (ComponentPortTreeModel)tree.getModel();
                        gModel.reset();
                        findDialog.reset();
                    ...
                    public void doUIUpdateLogic() {
                        int eventCode = -1;
                        if (hierarchy == ComponentRelationshipType.HOUSING)
                            eventCode = CableModelEvent.HOUSING_FIND_CONFIG;
                        else if (hierarchy == ComponentRelationshipType.CONTROL)
                            eventCode = CableModelEvent.CONTROL_FIND_CONFIG;
                        else
                            eventCode = CableModelEvent.POWER_FIND_CONFIG;

                        // add this selection request to Swing event queue
                        model.notifyCableModelListeners(new CableModelEvent(eventCode, findConfig));
                    }
                });
        }
    }
        ...
        public void actionPerformed(ActionEvent e) {
            ...
            QueuedExecutorUtil.execute(new SwingThreadWork(appFrame) {
                    public void doNonUILogic() {
                        ...
                        try {
                            ...
                            if (findConfig.getComponentId() != null) {
                                ComponentPortTreeModel tModel = (ComponentPortTreeModel)tree.getModel();
                    ...
                    public void doUIUpdateLogic() {
                        ...
                        // add this selection request to Swing event queue
                        model.notifyCableModelListeners(new CableModelEvent(eventCode, 
                                                                            findConfig));
                    }
                });
