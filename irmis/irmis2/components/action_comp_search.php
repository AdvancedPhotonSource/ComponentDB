<?php
	...
	$_SESSION['compReset'] = $_POST['compReset'];
	
	if ($_SESSION['compReset'] == "Reset") {
    ...
    if (!$ComponentTypeList->loadFromDB($conn, $_SESSION['nameDesc'], 
										...
										$_SESSION['personConstraint'], $ctID)) {
    ...
    if ($jmp==1) {
	$_GET['ID'] = $ctID;
	include_once ('action_comp_details_search.php');
	}
	else {
    include_once('comp_search_results.php');
	}
?>
