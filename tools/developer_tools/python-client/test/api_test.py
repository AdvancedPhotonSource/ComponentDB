...
from cdbApi import OpenApiException, ItemStatusBasicObject, NewLocationInformation, SimpleLocationInformation, \
    ...
    NewAppInformation, NewMAARCInformation
...
class MyTestCase(unittest.TestCase):
    ...
    def test_create_maarc_item(self):
        ...
        self.assertNotEqual(parent.id, None, msg="Top level item not created successfully")
        ...
        self.assertEqual(hierarchy_child_item.id, child.id, msg="Hierarchy fetched does not match what was created")

    def test_create_maarc_relationship(self):
        maarc_relationships = self.maarcApi.get_maarc_connection_relationship_list(self.MAARC_ITEM_ID)
        count_relationships = len(maarc_relationships)
        inital_count = 0

        self.assertEqual(count_relationships, inital_count, msg="Relationships to maarc item already exist.")

        self.loginAsAdmin()
        self.maarcApi.create_maarc_connection_relationship(self.MAARC_ITEM_ID, self.INVENTORY_ITEM_ID)

        maarc_relationships = self.maarcApi.get_maarc_connection_relationship_list(self.MAARC_ITEM_ID)
        count_relationships = len(maarc_relationships)

        self.assertEqual(count_relationships, inital_count +1, msg="New maarc relationship not found.")
        ...
        self.assertNotEqual(relevant_relationship, None, msg="Could not find newly added relationship.")
