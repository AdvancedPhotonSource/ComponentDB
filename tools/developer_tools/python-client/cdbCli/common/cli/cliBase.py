...
class CliBase:
    ...
    def require_authenticated_api(
        self, prompt_string="The command requires authentication.", portal_key=None
    ):
...
def cli_command_api_exception_handler(func):
    ...
    def Inner_Function(*args, **kwargs):
        if "cli" not in kwargs and "factory" not in kwargs:
            raise Exception(
                "'cli' or 'factory' must be a kwargs parameter to use decorator. Other recommended params are 'console' and 'format'"
            )
        try:
