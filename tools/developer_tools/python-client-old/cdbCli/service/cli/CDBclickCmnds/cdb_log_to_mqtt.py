...
def mqtt_on_connect(client, userdata, flags, rc):
    ...
    if rc == 0:
                ...
                client.publish(userdata["base_topic"] ,
...
def set_up_mqtt_client(clientname,
                       server,
                       port,
                       username,
                       password,
...
def get_cdb_logs(delta_minutes,logApi):
    ...
    results_list = [dict([("id",result.id),
                      ("text",result.text) ,
                ...
                for result in results if "Item Id" in result.text ]
    return(results_list)

@click.command()
@click.option("--cdb_server",default="https://cdb.aps.anl.gov/cdb",help="Address of the CDB Server")
@click.option("--mqtt_server",default="cooper.aps.anl.gov",help="Address of the MQTT Server")
@click.option("--mqtt_port",default=1883,help="TCP Port Number for the MQTT Server")
@click.option("--mqtt_clientname",default="cdb_log_to_mqtt",help="Unique MQTT clientname to identify this app")
@click.option("--mqtt_applicationtopic",default="CDBItemUpdate",help="Top level MQTT Topic under which messages are sent.")
@click.option("--sleepsecs",default=60,help="Time polling interval in seconds between sampling the CDB Logs")
#@click.argument("cdb_user")
#@click.argument("cdb_password")
@click.argument("mqtt_user")
@click.argument("mqtt_password")

def cdb_log_to_mqtt(cdb_server,
                    mqtt_server,
                    mqtt_port,
                    mqtt_clientname,
    ...
    mqtt_client = set_up_mqtt_client(mqtt_clientname,
                       mqtt_server,
                       mqtt_port,
                       mqtt_user,
                       mqtt_password,
...
#    apiFactory = CdbApiFactory(cdb_server)
    ...
    try:
        ...
        apiFactory = cli.require_authenticated_api("Authentication as an Administrator is required.")
    ...
    try:
        ...
        # to process
            new_log_messages = [ message for message in log_results
        ...
        # on it later.  We will just send the "now" time and the id.
            LOGGER.debug("In Loop, number of item_ids to send: " + str(len(item_ids_to_log)))
